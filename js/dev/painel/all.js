$(function(){
    ...
    **/
    ...
    $('body').on('change', '.bloco_video input', function(){
        ...
        if(valor.indexOf('youtube.com') != -1){
            ...
            $('.responsivo', bloco).html('<iframe src="https://www.youtube.com/embed/'+explode[explode.length-1]+'?rel=0&amp;showinfo=0" frameborder="0" allowfullscreen></iframe>');
            bloco.addClass('ativo');
	...
	**/
	$('body').on('click', '#but_add_geral', function(){
		...
		var booleano = $(this).attr('data-booleano') || '';
		var horarios = $(this).attr('data-historico') || 1;
    ...
    **/
    ...
    $('.adicionar_horarios').click(function(){
        var form = $(this).closest(".bloco_adicionado");
        var hora_de = $(".hora_de_horario", form).val();
        var hora_ate = $(".hora_ate_horario", form).val();
        var data_horario = $(".data_horario", form).val();
        var vinculo = $(this).attr("data-vinculo");
        var semana = [];
        $(".semana:checked", form).each(function(i){
            semana[i] = $(this).val();
        });
		...
		$.post(LINK+"/painel/horarios", {
            vinculo:vinculo,
			hora_de:hora_de,
			hora_ate:hora_ate,
			data_inicio:data_horario,
            semana:semana
		}, function(resposta){
			if(resposta.erro == false){
                $(".bloco_horarios_criados").load(LINK+"/painel/horarios_load/" + vinculo);
				$.alerta({notificacao:'Horario adicionado com sucesso.'});
                $(".hora_de_horario", form).val("");
                $(".hora_ate_horario", form).val("");
                $(".data_horario", form).val("");
                $(".semana:checked", form).each(function(i){
    ...
    $("body").on("click", ".remover_horario", function(){
        var bloco = $(this).closest(".bloco");
        var cod = $(this).attr("data-cod");
        $.post(LINK+"/painel/horarios_remover",{
            cod:cod
        }, function(resposta){
