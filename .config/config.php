...
final class config
{
    ...
    private function setDefines()
    {
        ...
        if (!empty($this->_header_imagem)) {
            define('IMAGEMSOCIAL', $this->_header_imagem);
        }
        ...
        if ($this->_imagem) {
            foreach ($this->_imagem as $ind => $valores) {
                define(mb_strtoupper($ind, 'UTF-8'), str_replace("{{LINK}}", $this->_link_link, $valores));
            }
        }

        if ($this->_social) {
            foreach ($this->_social as $ind => $valores) {
                define(mb_strtoupper($ind, 'UTF-8'), str_replace("{{LINK}}", $this->_link_link, $valores));
            }
        }

        if ($this->_opcionais) {
            foreach ($this->_opcionais as $ind => $valores) {
                define(mb_strtoupper($ind, 'UTF-8'), str_replace("{{LINK}}", $this->_link_link, $valores));
            }
        }
