...
class Upload
{
    public function enviar($file, $diretorio, $tipo, $ext_lista, $width = null, $height = null)
    {
        ...
        if (!$file) {
            return Mensagem::erro('Erro!', 'Nenhum arquivo enviado.');
        } elseif (in_array($file['error'], array(1, 2))) {
            return Mensagem::erro('Erro!', 'O arquivo enviado excede o limite de tamanho.');
        } elseif ($file['error'] == 4) {
            return Mensagem::erro('Erro!', 'Nenhum arquivo enviado.');
        } elseif (in_array($file['error'], array(3, 5, 6, 7, 8))) {
            return Mensagem::erro('Erro!', 'Erro no envio do arquivo.');
        } elseif ($file['error'] != 0) {
            return Mensagem::erro('Erro!', 'Erro no envio do arquivo.');
        } elseif (empty($diretorio)) {
            return Mensagem::erro('Erro!', 'Nenhum diretorio enviado.');
        } elseif (empty($tipo)) {
            return Mensagem::erro('Erro!', 'Nenhum tipo de procedimento enviado.');
        } elseif (!is_array($ext_lista) || empty($ext_lista)) {
            return Mensagem::erro('Erro!', 'Nenhuma extensão enviada.');
        } elseif (in_array($tipo, array('cortar', 'redimencionar')) && (empty($width) || empty($height))) {
            return Mensagem::erro('Erro!', 'Largura e altura são obrigatórios.');
        }

        $arquivo = $file['tmp_name'];
        $e_ext = explode('/', $file['type']);
        $ext = str_replace('jpeg', 'jpg', Converter::caixa(isset($e_ext[1]) ? $e_ext[1] : '', 'a'));
        $ext = str_replace('svg+xml', 'svg', Converter::caixa(isset($e_ext[1]) ? $e_ext[1] : '', 'a'));
    ...
    public static function gerar($bg, $imagem, $chamada, $rodape, $linha_01, $linha_02)
    {
        ...
        if (file_exists(DIRETORIO . '/post/' . $nome)) {
            return json_encode(array('erro' => false, 'imagem' => ARQUIVO . '/post/' . $nome, 'arquivo' => ARQUIVO . '/post/' . $nome, 'nome' => $nome));
        } else {
