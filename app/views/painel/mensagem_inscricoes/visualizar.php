...
<div id="bloco_visualizar_<?= $_app; ?>" class="app_visualizar_mensagem">
	...
	<div class="bloco_mensagem">
		...
		<div class="conteudo_mensagem">
			...
			<p class="linha"><span>EMAIL:</span> <span class="texto"><?= $r->email; ?></span></p>
			<?php endif; ?>
			...
			<?php if(!empty($r->curso->dados)): ?>
			<p class="linha"><span>CURSO:</span> <span class="texto"><?= $r->curso->dados->titulo; ?></span></p>
			<?php endif; ?>

			<?php if(!empty($r->turma)): ?>
			<p class="linha"><span>TURMA:</span> <span class="texto"><?= $r->turma; ?></span></p>
			<?php endif; ?>
			<div class="select but_change_atualizar" data-cod="<?= $r->cod; ?>" data-id="<?= $r->id; ?>" data-app="<?= $_app; ?>">
				<?php
				$Status = new StatusModel;
				echo Form::select('status', $Status->select('inscricoes', 'status'), $r->status->valor);
				?>
			</div>
		</div>
			<?php
