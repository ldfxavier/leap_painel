...
<div id="bloco_<?=$_app;?>" class="app_geral_lista">
    <header class="header_principal header_principal_animacao header_principal_absolute">
        ...
        <i data-font="&#xe816;" class="icones but_print" data-ajuda="IMPRIMIR DADOS"></i>
...
if ($lista):
    ...
    ?>
					<li class="li <?php echo ($selecao) ? 'but_visualizar_geral' : '' ?>" data-id="<?php echo $r->id ?>">
						<a href="<?=PAINEL;?>/visualizar/<?=$_app;?>/<?php echo $r->cod ?>">
							<div class="td"><?php echo $r->id ?></div>
							<div class="td"><?php echo P::filtro($r->nome->valor, 'Sem nome') ?></div>
							<div class="td"><?php echo P::filtro($r->email, 'Sem e-mail') ?></div>
							<div class="td"><?php echo P::filtro($r->documento->br, 'Sem Documento') ?></div>
							<div class="td status"><span class="status_<?php echo $r->status->valor ?>" data-ajuda="<?php echo $r->status->texto ?>"></span></div>
						</a>
					</li>
					<?php
endforeach;
