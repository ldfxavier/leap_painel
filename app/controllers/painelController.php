...
class painelController extends Controller
{
    ...
    public function horarios()
    {
        ...
        echo $Horarios->salvar($_POST);
        exit();
    }
    public function horarios_remover()
    {
        $Horarios = new HorariosModel;
        echo $Horarios->remover($_POST['cod']);
        exit();
    }
    public function horarios_load()
    {
        $Horarios = new HorariosModel;
        $cod = $this->getSep(2);
        ...
        foreach ($horarios as $r_h) :
            ...
            if (isset($r_h->semana->lista) && !empty($r_h->semana->lista)) :
                ...
                if ($count_semana <= 1) :
                    $retorno .= $r_h->semana->lista[0];
                elseif ($count_semana > 1) :
                    $retorno .= implode($r_h->semana->lista, " e ");
                endif;
            endif;
            $retorno .= "</p>
